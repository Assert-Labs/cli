...
/**
 * Generate the Pi extension file. Like OpenCode, Pi loads an in-process TS
 * module (via jiti), so we ship a thin shim: it maps Pi's extension events to
 * Assert events and forwards each to `assert hook pi <Event>` with a JSON
 * payload on stdin — putting Pi on the same subprocess model as every other
 * agent. `assertBin` is baked in as an absolute path (the module runs inside
 * Pi's process, where `$HOME` is not shell-expanded).
 */
export function generatePiExtension(version: string, assertBin: string): string {
  return `// Assert Pi extension — auto-generated by \`assert init\` (v${version}). Do not edit.
// Forwards Pi's in-process extension events to the Assert CLI so your coding
// session is captured for code attribution. https://github.com/Assert-Labs/cli
import { spawn } from "child_process"
...
// Fire-and-forget: never let a capture failure disrupt the agent. Each event is
// a short-lived \`assert hook pi <Event>\` process fed JSON on stdin.
function forward(event, payload) {
  return new Promise((resolve) => {
    let done = false
    const finish = () => { if (!done) { done = true; resolve() } }
    let child
    try {
      child = spawn(ASSERT_BIN, ["hook", "pi", event], { stdio: ["pipe", "ignore", "ignore"] })
    } catch {
      finish()
      return
    }
...
// Pi message/tool content is a string or an array of parts; pull the text out.
...
export default function (pi) {
  const cwd = (ctx) => (ctx && ctx.cwd) || process.cwd()
  const sid = (ctx) => {
    try { return ctx && ctx.sessionManager && ctx.sessionManager.getSessionId() } catch { return undefined }
  }
  const model = (ctx) => {
    const md = ctx && ctx.model
    if (!md) return {}
    const m = md.id || md.name || md.model
    const provider = typeof md.provider === "string" ? md.provider : (md.provider && md.provider.id)
    return { model: m, provider: provider }
  }

  pi.on("session_start", async (_event, ctx) => {
    await forward("SessionStart", { session_id: sid(ctx), cwd: cwd(ctx) })
  })

  pi.on("before_agent_start", async (event, ctx) => {
    const m = model(ctx)
    await forward("UserPromptSubmit", {
      session_id: sid(ctx), cwd: cwd(ctx),
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    await forward("PreToolUse", {
      ...
      call_id: event && event.toolCallId,
      model: m.model, provider: m.provider,
    })
  })

  pi.on("tool_result", async (event, ctx) => {
    const text = textOf(event && event.content)
    await forward("PostToolUse", {
      session_id: sid(ctx), cwd: cwd(ctx),
      tool_name: event && event.toolName,
      tool_input: (event && event.input) || {},
      call_id: event && event.toolCallId,
      tool_response: (event && event.isError) ? { error: text } : { output: text },
    })
  })
  ...
  pi.on("message_end", async (event, ctx) => {
    ...
    const m = model(ctx)
    await forward("AssistantText", {
      session_id: sid(ctx), cwd: cwd(ctx), text: text,
      model: m.model, provider: m.provider,
    })
  })
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    const m = model(ctx)
    await forward("Stop", { session_id: sid(ctx), cwd: cwd(ctx), model: m.model, provider: m.provider })
  })

  pi.on("session_shutdown", async (_event, ctx) => {
    await forward("SessionEnd", { session_id: sid(ctx), cwd: cwd(ctx) })
  })
}
`;
}
